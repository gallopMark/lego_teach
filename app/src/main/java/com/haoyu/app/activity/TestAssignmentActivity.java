    ...
    private void showContent(AppActivityViewResult response) {
        ...
        if (response.getResponseData() != null && response.getResponseData().getmActivityResult() != null) {
            ...
            if (detailMap != null) {
                if (state.equals("complete")) {
                    int score = (int) (detailMap.getResponse_score() / 10 + detailMap.getMark_score());
                    myhomeworkScore.setText(String.valueOf(score));
                    aresponseScore = String.valueOf((int) detailMap.getResponse_score() / 10);
        ...
        if (response.getResponseData() != null && response.getResponseData().getmAssignmentUser() != null) {
            ...
            if (response.getResponseData().getmAssignmentUser().getmAssignmentEntity() != null) {
                ...
                //作业被批阅的次数 0表示可以退回
                content = String.valueOf(mAssignmentEntity.getContent());
                ...
                if (running) {
                    ...
                    if (timePeriod != null) {
                        if (timePeriod.getEndTime() > 0) {
                            time = TimeUtil.convertToTime(timePeriod.getEndTime());
                ...
                if (mMarkType != null) {
                    ...
                    if (markType.equals("teach"))
                        markType.setText("老师评审");
                }
                already_assessment.setText("已互评" + response.getResponseData().getmAssignmentUser().getMarkNum() + "/" + mAssignmentEntity.getMarkNum() + "个");
                String state = response.getResponseData().getmAssignmentUser().getState();
