...
class WSHomePageActivity : BaseActivity(), View.OnClickListener, RecyclerTouchListener.RecyclerTouchListenerHelper {
    ...
    override fun initView() {
        ...
        recyclerView.layoutManager = layoutManager
        mAdapter = WSTaskEditAdapter(context, mDatas)
    ...
    private fun updateUI(response: WorkShopResult?) {
        if (response?.getResponseData() != null) {
            ...
            mWorkshop?.let { qualifiedPoint = it.qualifiedPoint }
            mWorkshopUser?.role?.let { roleInws = it }
    ...
    private fun playVideo(response: AppActivityViewResult, activity: CourseSectionActivity) {
        if (response.responseData.getmVideoUser() != null) {  //教学视频
            ...
            videoEntity.getmVideo()?.let {
                ...
                if (timePeriod?.state != null && timePeriod.state == "进行中") {
                    intent.putExtra("running", true)
                } else {
                    if (timePeriod != null && timePeriod.minutes > 0) {
                        intent.putExtra("running", true)
                    } else {
    ...
    public override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 1 && resultCode == Activity.RESULT_OK && data != null) {
            ...
            mAdapter.addActivity(activityIndex, activity)
        }
    }
    ...
    override fun onClick(v: View) {
        when (v.id) {
            ...
            R.id.tv_bottom -> smoothToBottom() //添加新阶段
        }
    }
    ...
    private fun showPopupWindow() {
        ...
        val pw = SupportPopupWindow(popupView, LinearLayout.LayoutParams.WRAP_CONTENT,
        ...
        //研修简报
        val llBrief = popupView.findViewById<View>(R.id.ll_brief)
        //学员测试
        val llStudenTest = popupView.findViewById<View>(R.id.ll_studen_test)
        //成员考核
        val llMm = popupView.findViewById<View>(R.id.ll_member_management)
        ...
        llBrief.setOnClickListener {
            pw.dismiss()
            val intent = Intent(context, BriefingActivity::class.java)
            intent.putExtra("relationId", workshopId)
            intent.putExtra("relationType", "workshop")
            intent.putExtra("type", "workshop_briefing")
            startActivity(intent)
        }
        llStudenTest.setOnClickListener {
            pw.dismiss()
            val intent = Intent(context, WSExamineActivity::class.java)
            intent.putExtra("workshopId", workshopId)
        ...
        llMm.setOnClickListener {
            pw.dismiss()
            val intent = Intent(context, WSManagerMebActivity::class.java)
            intent.putExtra("workshopId", workshopId)
            startActivity(intent)
        }
